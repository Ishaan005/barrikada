...
def safe_decode(
    ...
    if len(raw_bytes) > 0:
        try:
            ...
            logger.debug(
                f"Chardet detected: {chardet_result.get('encoding')} (confidence: {detection_confidence:.2f})"
            )
    ...
    if chardet_result and detection_confidence >= confidence_threshold:
        ...
        if guessed_enc and guessed_enc.lower() not in [enc.lower() for enc in attempted_encodings]:
            try:
    ...
    suspicious = (
        best_replacements > suspicious_threshold
        or utf8_decode_errors > suspicious_threshold
        or (
            detection_confidence > 0 and detection_confidence < 0.3
    ...
    if best_replacements > 0:
        logger.info(
            f"Decode had {best_replacements} replacement characters (encoding: {best_encoding})"
        )
