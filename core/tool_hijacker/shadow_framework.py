...
class ShadowLLM:
    ...
    def select_tool(self, task_description, retrieved_tools, return_reasoning=False):
        ...
        return {
            ...
            "reasoning": f"Selected based on keyword overlap score: {best_score}"
            if return_reasoning
            else None,
        }
...
class ShadowFramework:
    ...
    def execute_pipeline(self, task_description, top_k=5):
        ...
        """
        # Step 1: Retrieval
        retrieved_tools = self.retriever.retrieve(task_description, self.shadow_tools, top_k)

        # Step 2: Selection
        selection_result = self.llm.select_tool(
            task_description, retrieved_tools, return_reasoning=True
        )
