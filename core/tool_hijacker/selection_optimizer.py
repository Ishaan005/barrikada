...
class SelectionNode:
    ...
    children: list["SelectionNode"] | None = None
...
class GradientFreeSelectionOptimizer(SelectionOptimizer):
            ...
            for node in current_level:
                ...
                for variant in variants:
                    ...
                    success_count = self._evaluate_s_variant(
                        variant, malicious_tool, retrieved_tools, task_queries
                    )
                ...
                if node.children is not None:
                    node.children = [
                        child for child in node.children if child.success_rate >= prune_threshold
                    ]
    ...
    def _generate_s_variants(
        self, current_s, malicious_tool, retrieved_tools, num_variants, feedback=""
    ):
        """
        ...
        """
        prompt = self._create_attacker_prompt(current_s, malicious_tool, retrieved_tools, feedback)
...
Provide only the S variants, one per line:"""
    ...
    def _create_feedback(self, node):
        ...
        if success_rate < 0.3:
            ...
            return (
                "Previous variants worked well. Generate similar variants with slight variations."
            )
    ...
    def _evaluate_s_variant(self, s_variant, malicious_tool, retrieved_tools, task_queries):
        ...
        test_tools = [temp_tool if t.name == malicious_tool.name else t for t in retrieved_tools]
...
class GradientBasedSelectionOptimizer(SelectionOptimizer):
        ...
        **kwargs,
        ...
        for iteration in range(num_iterations):
            ...
            current_loss = self._compute_loss(
                current_s, malicious_tool, retrieved_tools, task_queries, target_tool_name
            )
            ...
            for token_idx in range(len(s_tokens)):
                ...
                for candidate_token in self.vocabulary:
                    ...
                    candidate_loss = self._compute_loss(
                        candidate_s, malicious_tool, retrieved_tools, task_queries, target_tool_name
                    )
    ...
    def _compute_loss(
        self, s_text, malicious_tool, retrieved_tools, task_queries, target_tool_name
    ):
        """
