...
from .retrieval_optimizer import GradientBasedRetrievalOptimizer, GradientFreeRetrievalOptimizer
from .selection_optimizer import GradientBasedSelectionOptimizer, GradientFreeSelectionOptimizer
...
class ToolHijacker:
    ...
    def __init__(
        self, shadow_retriever=None, shadow_llm=None, task_generator=None, tool_library=None
    ):
        """
        ...
        self.shadow_framework = ShadowFramework(self.shadow_retriever, self.shadow_llm)
    ...
    def generate_attack(
        ...
        num_shadow_tasks=10,
        num_shadow_tools_relevant=10,
        num_shadow_tools_irrelevant=20,
        top_k_retrieval=5,
        **optimizer_kwargs,
    ):
        """
        ...
        print("[ToolHijacker] Constructing shadow framework...")
        self._construct_shadow_framework(
            target_task, num_shadow_tasks, num_shadow_tools_relevant, num_shadow_tools_irrelevant
        )
        ...
        print("[ToolHijacker] Phase 1: Optimizing retrieval subsequence (R)...")
        r_optimized = self._optimize_retrieval(
            malicious_tool, optimization_method, **optimizer_kwargs
        )
        ...
        print("[ToolHijacker] Phase 2: Optimizing selection subsequence (S)...")

        # Get typical retrieved set for S optimization
        sample_retrieved = self.shadow_retriever.retrieve(
            self.shadow_tasks[0].query, self.shadow_framework.shadow_tools, top_k_retrieval
        )

        s_optimized = self._optimize_selection(
            malicious_tool, sample_retrieved, optimization_method, **optimizer_kwargs
        )
        ...
        print("[ToolHijacker] Evaluating attack success...")
        metrics = self.shadow_framework.evaluate_attack_success(
            [task.query for task in self.shadow_tasks], malicious_tool_name, top_k_retrieval
        )
        ...
        print("[ToolHijacker] Attack generation complete!")
    ...
    def _construct_shadow_framework(
        self, target_task, num_shadow_tasks, num_relevant_tools, num_irrelevant_tools
    ):
        """
        ...
        self.shadow_tasks = self.task_generator.generate(
            target_task, num_variations=num_shadow_tasks
        )

        # Build shadow tool library (D')
        self.shadow_tools = self.tool_library.build_default_library(
            num_relevant=num_relevant_tools, num_irrelevant=num_irrelevant_tools
        )
    ...
    def _optimize_retrieval(self, malicious_tool, method, **kwargs):
        ...
        if method == "gradient_free":
            optimizer = GradientFreeRetrievalOptimizer(
                self.shadow_retriever, llm_generator=kwargs.get("llm_generator")
            )
        elif method == "gradient_based":
            optimizer = GradientBasedRetrievalOptimizer(
                self.shadow_retriever, vocabulary=kwargs.get("vocabulary")
            )
        elif method == "hybrid":
            ...
            r_initial = optimizer_gf.optimize(self.shadow_tasks, malicious_tool, **kwargs)

            optimizer_gb = GradientBasedRetrievalOptimizer(
                self.shadow_retriever, vocabulary=kwargs.get("vocabulary")
            )
            return optimizer_gb.optimize(
                self.shadow_tasks, malicious_tool, initial_r=r_initial, **kwargs
            )
        else:
            raise ValueError(f"Unknown optimization method: {method}")

        return optimizer.optimize(self.shadow_tasks, malicious_tool, **kwargs)
    ...
    def _optimize_selection(self, malicious_tool, retrieved_tools, method, **kwargs):
        ...
        if method == "gradient_free":
            optimizer = GradientFreeSelectionOptimizer(
                self.shadow_llm, attacker_llm=kwargs.get("attacker_llm")
            )
        elif method == "gradient_based":
            optimizer = GradientBasedSelectionOptimizer(
                self.shadow_llm, vocabulary=kwargs.get("vocabulary")
            )
        elif method == "hybrid":
            # Use gradient-free first, then refine with gradient-based
            ...
            s_initial = optimizer_gf.optimize(
                self.shadow_tasks, malicious_tool, retrieved_tools, **kwargs
            )
        ...
        return optimizer.optimize(self.shadow_tasks, malicious_tool, retrieved_tools, **kwargs)
    ...
    def test_attack(self, malicious_tool, test_tasks, top_k=5):
        ...
        # Evaluate
        return self.shadow_framework.evaluate_attack_success(test_tasks, malicious_tool.name, top_k)
