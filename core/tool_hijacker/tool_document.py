...
class ToolDocument:
    ...
    def to_dict(self):
        ...
        return {"name": self.name, "description": self.description, "metadata": self.metadata}

    @classmethod
    def from_dict(cls, data):
        """Create ToolDocument from dictionary"""
        return cls(
            name=data["name"], description=data["description"], metadata=data.get("metadata", {})
        )
...
class MaliciousToolDocument(ToolDocument):
    ...
    retrieval_subsequence: str | None = None  # R in the paper
    selection_subsequence: str | None = None  # S in the paper
