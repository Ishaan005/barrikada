...
class TestbedGenerator:
    ...
    def generate_dataset(self, num_benign=50, num_malicious=50):
        ...
        """
        print(f"\n{'=' * 60}")
        print("🔧 TESTBED GENERATOR (LLM-Powered)")
        print(f"{'=' * 60}")
    ...
    def _generate_malicious(self, index):
        ...
        description = self.factory.create_malicious_tool(category, tool_name, target)
        if not description:
    ...
    def _save_dataset(self, tools):
        ...
        with open(filename, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                f, fieldnames=["name", "category", "description", "label", "target_behavior"]
            )
            writer.writeheader()
