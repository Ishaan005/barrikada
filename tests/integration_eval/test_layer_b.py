...
def test_layer_b():
        ...
        results.append(
            {
                ...
                "layer_b_top_similarity": max(
                    (m.confidence for m in layer_b_result.matches), default=0.0
                ),
    ...
    # Verdict breakdown
    verdict_counts = (
        results_df["layer_b_verdict"].fillna("unknown").astype(str).str.lower().value_counts()
    )
    ...
    safe_allow = (
        (results_df["true_label"] == 0) & (results_df["layer_b_verdict"] == "allow")
    ).sum()
    safe_flag = ((results_df["true_label"] == 0) & (results_df["layer_b_verdict"] == "flag")).sum()
    safe_block = (
        (results_df["true_label"] == 0) & (results_df["layer_b_verdict"] == "block")
    ).sum()

    malicious_allow = (
        (results_df["true_label"] == 1) & (results_df["layer_b_verdict"] == "allow")
    ).sum()
    malicious_flag = (
        (results_df["true_label"] == 1) & (results_df["layer_b_verdict"] == "flag")
    ).sum()
    malicious_block = (
        (results_df["true_label"] == 1) & (results_df["layer_b_verdict"] == "block")
    ).sum()
    ...
    # Print SAFE row
    print(
        f"{'SAFE':<15} | {safe_allow:>10} | {safe_flag:>10} | {safe_block:>10} | {safe_total:>10}"
    )

    # Print MALICIOUS row
    print(
        f"{'MALICIOUS':<15} | {malicious_allow:>10} | {malicious_flag:>10} | {malicious_block:>10} | {malicious_total:>10}"
    )
    ...
    print(
        f"  Correct:   {correct_decisive}  (mal->block: {malicious_block}, safe->allow: {safe_allow})"
    )
    print(
        f"  Incorrect: {incorrect_decisive}  (safe->block: {safe_block}, mal->allow: {malicious_allow})"
    )
