...
def test_pipeline():
    ...
    for _, row in test_case_df.iterrows():
        test_cases.append({"label": row["label"], "text": row["text"]})
    ...
    for test_case in test_cases:
        ...
        if result.final_verdict == "allow" and test_case["label"] == 0:
            correct += 1
            is_correct = True
        elif (result.final_verdict == "block" or result.final_verdict == "flag") and test_case[
            "label"
        ] == 1:
            correct += 1
            is_correct = True
        ...
        results.append(
            {
                ...
                "layer_a_flags": "; ".join(
                    result.layer_a_result.get("flags", []) if result.layer_a_result else []
                ),
                "layer_b_matches": len(
                    result.layer_b_result.get("matches", []) if result.layer_b_result else []
                ),
                "layer_c_verdict": result.layer_c_result.get("verdict", "")
                if result.layer_c_result
                else "",
                "layer_c_probability": result.layer_c_result.get("probability_score", 0.0)
                if result.layer_c_result
                else 0.0,
                "layer_c_confidence": result.layer_c_result.get("confidence_score", 0.0)
                if result.layer_c_result
                else 0.0,
            }
        )
