...
def _layer_d(
    texts: Sequence[str], candidate_dir: Path
) -> tuple[tuple[list[Verdict], float], tuple[list[Verdict], float]]:
    outputs = ROOT / "core/layer_d/outputs"
    settings = Settings()
    with tempfile.TemporaryDirectory(prefix="barrikade-layer-d-reference-") as directory:
        reference_path = Path(directory) / "model"
        reference_path.symlink_to(outputs / "model", target_is_directory=True)
    ...
    with tempfile.TemporaryDirectory(prefix="barrikade-layer-d-candidate-") as directory:
        ...
        (candidate_root / "model").symlink_to(outputs / "model", target_is_directory=True)
...
def main() -> int:
    ...
    evaluators = {
        "layer_b": _layer_b,
        "layer_c": _layer_c,
        "layer_d": lambda values: _layer_d(values, args.layer_d_candidate_dir),
    }
