...
def print_layer_status(layer_name: str, local_status: dict, gcs_status: dict):
    ...
    if local_status["exists"]:
        print(
            f"  Current: {len(local_status['current_files'])} file(s), {format_size(local_status['current_size'])}"
        )
        ...
        if local_status["archives"]:
            ...
            for archive_name, archive_info in sorted(
                local_status["archives"].items(), reverse=True
            )[:3]:
                print(
                    f"    - {archive_name}: {len(archive_info['files'])} file(s), {format_size(archive_info['size'])}"
                )
    ...
    if gcs_status.get("error"):
        ...
        print(
            f"  Current: {len(gcs_status['current_files'])} file(s), {format_size(gcs_status['current_size'])}"
        )
        ...
        if gcs_status.get("archives"):
            ...
            for archive_name, archive_info in sorted(gcs_status["archives"].items(), reverse=True)[
                :3
            ]:
                print(f"    - {archive_name}: {len(archive_info['files'])} file(s)")
    else:
        print("  ✗ No models found on GCS")
...
def main():
    parser = argparse.ArgumentParser(description="Show status of local and GCS models")
    parser.add_argument(
        "--bucket",
        required=True,
