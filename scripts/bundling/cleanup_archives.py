...
def clean_local_archives(layer_name: str, keep: int = 3, dry_run: bool = False) -> dict:
    ...
    for old_version_path, _ in versions[keep:]:
        try:
            ...
            if dry_run:
                logger.info(
                    f"  [DRY RUN] Would delete: {old_version_path.name} ({format_size(total_size)})"
                )
            else:
    ...
    try:
        ...
        for archive_prefix, archive_name in versions[keep:]:
            try:
                ...
                if dry_run:
                    logger.info(
                        f"  [DRY RUN] Would delete: {archive_name} ({format_size(total_size)})"
                    )
                else:
...
def main():
    parser = argparse.ArgumentParser(description="Clean up old archived model versions")
    parser.add_argument(
        "--keep",
        type=int,
        default=3,
    ...
    for name, result in results.items():
        if result["errors"]:
            ...
            logger.info(
                f"✓ {name}: {result['deleted_versions']} version(s) deleted, {format_size(result['freed_space'])} freed"
            )
