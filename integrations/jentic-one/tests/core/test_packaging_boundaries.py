...
def test_thin_plugin_has_only_approved_runtime_dependencies():
    project = tomllib.loads((INTEGRATION_ROOT / "pyproject.toml").read_text())["project"]
...
def test_thin_plugin_never_imports_core_or_model_runtimes():
    ...
    imported = set()
    for source_file in (INTEGRATION_ROOT / "src" / "barrikade_jentic").rglob("*.py"):
...
def test_overlay_is_pinned_and_contains_only_the_plugin_wheel():
    dockerfile = (INTEGRATION_ROOT / "deploy" / "overlay.Dockerfile").read_text()
...
def test_compose_exposes_one_public_barrikade_switch():
    compose = yaml.safe_load((INTEGRATION_ROOT / "deploy" / "compose.yaml").read_text())
